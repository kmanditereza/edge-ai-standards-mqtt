...
Example: site:area:line:cell/edge_nodeID/device/ID/raw_namespace/prediction/model_id/model_version
...
↪️ Device
  ...
  ↪️ “raw/temperature” (json) -> This raw value from a sensor is in the "Raw Data" namespace and is consumed by a model

  ↪️ “predictions/model_id/version” (json) -> This funtional value is a predicted value from a specific model and version

  ↪️ SafetyViolation (boolean) -> This functional value is the business metric that is composed from raw sensor data and one or more models to create a meaningful insight out of this data.

Example:
* Raw data: Camera feed
* Funtional prediction "people": "3 people detected"
* Functional prediction "hardhats": "2 hard hats detected"
* Function business metric "SafetyViolation": "1 person isn't wearing a hardhat"
...
For Flat MQTT applications, we propose the following general payload object. This would be common accross all types of models. The `results` object could then be customized to contain the model-specific data struture. In a JSON format, this model data structure would appear as follows.
...
Results can be provided in one of three ways:
...
Below are recommended JSON structures for the most common types of machine learning models.
...
#### Image segmentation
Image segmentation is used to highlight specific pixesl within an image or a video frame that are of importance.
