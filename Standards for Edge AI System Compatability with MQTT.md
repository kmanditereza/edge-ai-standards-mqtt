...
One of the most important parts of setting up any MQTT system is designing the topic structure. There are many ways to define such a structure including the ISA-95 standard or Sparkplug. The standards provided in this document can work in conjunction with either of these standards, or with custom-defined topic structures. The focus in this document is to establish recommendations for logical naming conventions that seemlessly integrate machine learning into live MQTT systems.

### Namespace  Definition
A key step in topic namespace structuring is the identification of edge AI namespaces. The following two namespace categories provide a simple and logical mechanism for organizing the different types of information used by and generated by AI systems:
 * `raw` data namespace
...
There are two functional namespace categories that should be utilized when building edge AI systems on top of MQTT: `inference` and `insight`
