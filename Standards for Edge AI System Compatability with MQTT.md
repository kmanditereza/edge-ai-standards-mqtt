...
Messages to this topic might include the following metrics: Air temperature (K), Process temperature (K), Rotational speed (rpm), Torque (Nm), Tool wear (min)
...
1. **`inference`**: Used for publishing the raw inferences generated by AI/ML models
2. **`insight`**: Used for publishing business metrics derived from underlying AI/ML models

#### Inference Namespace
This namespace element refer to metrics directly generated by a machine learning model. These outputs are highly transactional and do not necessarily provide business value on their own. In the context of computer vision, messages published to an "inference" metric might count the number of people and hardhats detected within a video frame.

**Example**

The `inference` namespace for the same milling machine might be used to publish AI predictions to the following topic:

`site/area/line/cell/milling_machine/Machine Failure Prediction/0.0.1/inference`

Messages to this topic might include the following metrics: `failure` and `no_failure` classifications along with corresponding scores generated by a machine learning model.


#### Insight Namespace
This category of topics would refer to metrics with innate business value that are composed, calculated, or otherwise built on top of individual inference metrics. Using the computer vision example from above, an "insight" metric might subtract the number of hardhats from the number of people in a video frame to determine if someone is not wearing protective equipment.

**Example**

The "insight" namespace for this milling machine might be used to publish business rules or other insights that are generated or calculated based on an `inference`: 

`site/area/line/cell/milling_machine/Machine Failure Prediction/0.0.1/insight`
 
Messages to this topic might include a boolean valued called `maintenance_required`, which would be published anytime the `inference/failure` score is larger than the `inference/no_failure` score, indicating that the Machine Failure Prediction model has found the milling machine to be likely to fail some time soon.
...
**device_id**: While often used to to indicate a physical or logical device, such as a PLC, this field can also be used to represent a logical grouping of data points. Using this latter definition, device_id should be set to the name of the model that is generating inferences. A human-readable model name (i.e. `Machine Failure Prediction`) is a very readable way to structure this value, however care should be taken to choose a value for the model name that is stable. This may mean that using a unique identifier is actually the best choice (i.e. `brzrip6cxk`) so that a change in model name doesn't impact downstream applications.
...
&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; └ "model_version/insight/business_metric" -> This functional value is the business metric that is composed of raw data processed through one or more models, and then refined to create a meaningful insight out of this data.
...
2. Structured insights (Classification or Detection) that are extracted from unstructured sources like audio and video
