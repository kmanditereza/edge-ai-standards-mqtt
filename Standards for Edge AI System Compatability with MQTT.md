...
Using the  milling machine example, the "insight" namespace for this might be used to publish business rules that are generated or calculated based on an `inference`: 
...
**[Customized Edge Hardware Identifier]**: A unique identifier of some kind, pointing to the location of the device upon which an AI/ML model is running.

**model_name**: The name of a specific AI/ML model. A human-readable model name (i.e. `Machine Failure Prediction`) is a very readable way to structure this value, however care should be taken to choose a value for the model name that is stable and not likely to change. This may mean that using a unique identifier is actually the best choice (i.e. `brzrip6cxk`) so that a change in model name doesn't impact downstream applications.
...
**device_id**: While often used to to indicate a physical or logical device, such as a PLC, this field can also be used to represent a logical grouping of data points. Using this latter definition, **use this element to indicate the identity of the AI/ML model** that is generating inferences. A human-readable model name (i.e. `Machine Failure Prediction`) is a very readable way to structure this value, however care should be taken to choose a value for the model name that is stable and not likely to change. This may mean that using a unique identifier is actually the best choice (i.e. `brzrip6cxk`) so that a change in model name doesn't impact downstream applications.
