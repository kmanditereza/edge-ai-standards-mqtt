...
Models consuming data in this pattern may include applications like predictive maintenance, as part of an operational control feedback system, or for general time-series classification.

### The "Unstructured Data" pattern
This pattern represents an AI/ML system where input data is available via some other system or protocol, that raw data is then processed through a model, and finally predictions are published to MQTT.
...
This pattern represents an AI/ML system where inputs data is published to MQTT and processed by one or more AI & ML models, but is then being sent to another system via a different protocol.

AI/ML systems operating in this pattern may include applications such as robotics, where sensor data is used to generate a prediction, which is then fed directly to a control system.
